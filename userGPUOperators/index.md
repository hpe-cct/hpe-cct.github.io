...
rules, like this:
    ...
    def square(v: VectorField): VectorField =
      GPUOperator(v.fieldType) {
        if (isBigTensorField(v.fieldType.tensorShape)) {
          // Big tensor kernel
          _forEachTensorElement(v.tensorShape) {
            val element = _tensorElementVar(v)
...
The `if (isBigTensorField…)` code runs in Scala, not on the GPU, so it does
...
| `isBigTensorField(Shape)` |      Returns true if the specified shape (assumed to be a tensor shape) is a “big tensor.”
